...
- **Wire schemas are deliberately narrower than local schemas** — `describeForWire()` admits unsupported node kinds only when their complete inspectable graph has no secret role. A union, intersection, transform, lazy schema, or other structure that reaches `role('secret')` is omitted instead of partially described; lazy builders are never executed for the wire safety proof. The gateway rejects browser writes for omitted namespaces. Keep browser-editable secrets in the supported `object`/`dict`/`array` paths or expose a non-secret reference instead.
