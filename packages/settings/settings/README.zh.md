...
- `register(ns, schema, { base?, applies?, exposeToClients? })` — 返回 owner 的 `SettingsScope`（`get`/`watch`/`update`）。注册是调用方插件 fiber 上的 effect：dispose（资源释放）该 fiber 即移除 namespace 及其观察者。`exposeToClients` 默认 `false`；它只允许部署配置浏览器暴露，绝不自行授予该权限。schema 拒绝的存量分节会使注册本身失败；重复 namespace 立即报错。
- `describe(options?)` — 每个 namespace 一条描述（`schema.toJSON()` 封装、解析值、分离出的 `base`/`user` 层、`applies`），供进程内配置界面使用；字段出现在 `user` 中即标记其被用户覆盖。`describe({ redactSecrets: true })` 从每一层剥离 `role('secret')` 字段，并附加 `secrets` slot 列表（`{ path, set }`）。浏览器协议使用 `describeForWire()`：它会脱敏每一层、移除 secret schema 默认值，并省略无法证明所有 secret 路径安全的 namespace。纯遍历器 `redactSecrets(schema, value)` 仍供非 wire 调用方使用。
...
外部 settings namespace 需要两项独立授权：其属主调用 `ctx.settings.register(settingsNamespace('my-ui-plugin'), schema, { exposeToClients: true })`，随后部署在 API gateway 配置中列出同一名称。网关保留产品持有和 LLM 提供方 namespace 的既有规则；此额外列表不会扩大它们，并可接受稍后才加载的插件名称。
...
`isExposedToClients(ns)` 只报告仍在运行的注册所作的 opt-in，因此卸载属主会撤销它。缺少任一授权、namespace 未注册，或 schema 无法被 `describeForWire()` 证明安全时，网关都会回答 `settings-not-exposed`；它们都不会泄露注册表。
