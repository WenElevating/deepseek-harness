...
dsh 桌面 surface 组合包。[`cordis.patch.yml`](cordis.patch.yml) 叠加在 [`dsh-web-app`](../web-app/README.md) 之上，作为 `electron` profile 模板的第三层：它禁用 HTTP 载体家族（[`dsh-host-webserver`](../../host/webserver/README.md) 与重载链 [`dsh-client-hmr`](../../client/hmr/README.md)——没有 HTTP surface 可服务时它们只是死重）和自适应的 [`directory-picker`](../../host/directory-picker-auto/README.md) 条目，插入 Electron 替代品——[`dsh-host-directory-picker-electron`](../../host/directory-picker-electron/README.md)（harness 运行在 Electron 主进程内，OS 对话框无需跨进程跳转）、与之配对的 [`dsh-client-ui-directory-picker-native`](../../client/ui-directory-picker-native/README.md) surface（禁用自适应条目会连带丢弃其解析交互的两个半边，缺少这个占位者时侧边栏与空态页都不提供添加工作区动作）、以及 [`dsh-host-connection-electron`](../../host/connection-electron/README.md)（IPC 载体，认领载体中立的 `connection` 宿主半边并绑定到 `ipcMain`/`webContents`）——并保留全部 `dsh.client` 浏览器条目，因为桌面 shell 通过 Electron 的 renderer 加载同一前端。本包没有运行时 API；profile 组合器通过 `dsh.bundle.patch` 清单字段解析 patch，从不经过代码。
