...
**Product exposure stays a Host allowlist; external exposure needs two approvals.** The three namespaces remain in `WEB_SETTINGS_NAMESPACES`, so their product behavior is unchanged. An external owner declares `exposeToClients: true`, then its deployment lists the namespace in `api-gateway.exposedSettingsNamespaces`; either missing approval answers `settings-not-exposed` exactly as an unregistered namespace does. The deployment-controlled rule is recorded in [the external settings authorization note](../architecture/2026-08-18-external-settings-deployment-authorization.md).
...
- **A deployment allowlist without a registration-time declaration.** The deployment could accidentally expose a third-party plugin that did not intend a browser configuration path. The owner declaration remains a required authorization.
- **A registration-time declaration without deployment configuration.** A composed plugin could then enlarge the deployment's browser configuration access. The profile remains a required authorization.
...
Two costs are real. An external plugin needs both an owner declaration and a deployment profile entry before its card can load, so a client plugin cannot make itself visible on an arbitrary Host. And the plugins the web deployment moved into the agent plane — the file tools, the skills, compaction, the todo tool — appear nowhere here, which is most of what a user might expect to find; their configuration remains the preset editor's.
