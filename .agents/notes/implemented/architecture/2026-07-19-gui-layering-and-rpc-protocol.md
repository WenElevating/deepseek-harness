...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - The Electron desktop shell (`apps/electron`) reuses the same web client packages over the [IPC fetch carrier](2026-08-15-electron-ipc-desktop-shell.md).
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (e.g. the Electron IPC carrier, see the "Subclass table" below).
...
| `IpcApiClient` | dsh-client-connection | the `window.__DSH_IPC__` bridge (`dsh:fetch` invoke) | the Electron desktop shell's renderer carrier ([desktop shell note](2026-08-15-electron-ipc-desktop-shell.md)); swaps `doFetch` and the two stream openers; contract and base class unchanged |
