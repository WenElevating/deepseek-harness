...
**卡片由 settings 命名空间拥有，但暴露仍需部署授权。** `settings.plugin.item` 卡片注册表以所编辑的命名空间为键，但只有 Host 实际服务该命名空间时才会派发卡片。外部命名空间必须同时满足[外部 settings 授权 note](2026-08-18-external-settings-deployment-authorization.md)所述的存活插件 opt-in 与部署 `exposedSettingsNamespaces` 清单；未注册或未授权的名字回答 `settings-not-exposed`，不安全的 schema 则由 `describeForWire()` 省略。

**settings seam 只拥有插件 opt-in，不拥有部署决定。** `SettingsRegisterOptions.exposeToClients` 记录插件作者已批准浏览器使用；部署 profile 提供独立的 allowlist。哪个页面渲染已获授权的命名空间仍是 Consumer 的事实；Service Definition 若携带页面选择，就等于让一个 Consumer 决定契约。
