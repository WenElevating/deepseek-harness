...
**Cards are owned by the settings namespace, while exposure remains deployment-authorized.** The `settings.plugin.item` card registry is keyed by the namespace it edits, but a card is dispatched only when the Host serves that namespace. External namespaces must satisfy both the live plugin opt-in and the deployment `exposedSettingsNamespaces` list described in the [external settings authorization note](2026-08-18-external-settings-deployment-authorization.md); unregistered or unauthorized names answer `settings-not-exposed`, and unsafe schemas are omitted by `describeForWire()`.

**The settings seam owns the plugin opt-in, not the deployment decision.** `SettingsRegisterOptions.exposeToClients` records that the plugin author approved browser use; the deployment profile supplies the independent allowlist. Which page renders an authorized namespace remains a consumer fact; a Service Definition that carried the page choice would let one Consumer dictate its contract.
