...
A **profile** is a named composition stored in the Harness home. It lists the bundles it stacks, holds any out-of-tree plugins it installs, and keeps the user's own `cordis.patch.yml`. `web`, `electron`, and `headless` ship as templates.
...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-electron-app`](../packages/bundle/electron-app/README.md) rides over it for the desktop shell, trading the HTTP carrier family for the Electron IPC carrier ([desktop shell note](../.agents/notes/implemented/architecture/2026-08-15-electron-ipc-desktop-shell.md)); [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
